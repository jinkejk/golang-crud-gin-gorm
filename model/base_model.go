...
import (
	...
	"golang-crud-gin/global/variable"
...
func DatabaseConnection() *gorm.DB {
	if variable.GormDbMysql == nil {
		sqlInfo := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=utf8mb4&parseTime=True&loc=Local", config.User, config.Password, config.Host, config.Port, config.DbName)
